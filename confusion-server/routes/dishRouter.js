    ...
    .then(dish => {
        if(dish){
            ...
            if(!dish.comments.id(req.params.commentId).author.equals(req.user._id) ){
                let err = new Error('You are not allow to edit this comment: ' + req.params.commentId);
                err.statusCode = 401;
                return next(err);
            }
...
.delete(authenticate.verifyUser, (req, res, next) => {
    ...
    .then(dish => {
        if(dish){
            ...
            if(!dish.comments.id(req.params.commentId).author.equals(req.user._id)){
                let err = new Error('You are not allow to delete this comment: ' + req.params.commentId);
                err.statusCode = 401;
                return next(err);
            }
